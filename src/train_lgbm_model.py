...
# import xgboost as xgb # Comment out XGBoost
...
def main():
    ...
    # --- 5. Model Training ---
    # print("--- Training XGBoost Regressor Model ---") # Comment out XGBoost print
    # model = xgb.XGBRegressor(objective='reg:squarederror', # Comment out XGBoost model
    #                            n_estimators=100,
    #                            random_state=42,
    #                            n_jobs=-1,
    #                            max_depth=6,
    ...
    print("--- Training LightGBM Regressor Model ---") # New print for LightGBM
    ...
    # model_path = os.path.join(models_path, 'xgboost_model.joblib') # Comment out XGBoost model path
    model_path = os.path.join(models_path, 'lightgbm_model.joblib') # New model path for LightGBM

    try:
        joblib.dump(preprocessor, preprocessor_path)
        joblib.dump(model, model_path)
        print(f"Preprocessor saved to: {preprocessor_path}")
        # print(f"Trained XGBoost model saved to: {model_path}") # Comment out XGBoost save print
        print(f"Trained LightGBM model saved to: {model_path}") # New save print for LightGBM
